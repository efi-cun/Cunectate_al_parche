...
### B. Estructura y Cálculo del Tablero HTML (Diseño Diapositiva 1)
- **Fondo**: Canvas intergaláctico fluido no invasivo (`#galaxy-canvas`).
- **Hero Card Principal Top (100% Ancho)**: Muestra `TOTAL REGISTROS ÚNICOS VÁLIDOS` con tipografía gigante gradiente y **Barra de Progreso Animada que calcula dinámicamente el % de asistencia sobre el Total de la Planta (ej. Registros Únicos / Total Planta de 1.188 Facilitadores)**.
- **Subgrid KPI (Justo debajo)**: 
  - Tarjeta 1: **ASISTENCIA PRESENCIAL** (verde esmeralda neón, clicable con modal por regiones).
  - Tarjeta 2: **ASISTENCIA VIRTUAL** (morado neón).
- **Panel de Escuelas Dividido en 2 Columnas**:
  - **Columna Izquierda (`escuelas-panel`)**: Nombre de la escuela, **Total de Asistentes** y barra de porcentaje de participación.
  - **Columna Derecha (`escuelas-panel`)**: Nombre de la escuela y **Desglose de cuántos son Presenciales y cuántos son Virtuales**.
- **Buscador Superior**: Filtra sincrónicamente ambas columnas en tiempo real.
...
- En las hojas `Consolidado_Unicos` y `Duplicados`, los campos recuperados de Planta contienen la fórmula **desplegada a lo largo de 1.500 filas**:
  - `INVITACIÓN` (Col A): `=IF(BN="","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 2, FALSE), "NO ENCONTRADO EN PLANTA"))`
  - `CEDULA` (Col B): Número de Cédula de asistencia.
  - `NOMBRE` (Col C): Nombre limpio de asistencia o `=IF(BN="","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 3, FALSE), ""))`
  - `Nombre Nivel 2` (Col D): `=IF(BN="","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 5, FALSE), "NO ENCONTRADO EN PLANTA"))`
  - `Nombre Nivel 3` (Col E): `=IF(BN="","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 6, FALSE), "NO ENCONTRADO EN PLANTA"))`
  - `Descripción Cargo` (Col F): `=IF(BN="","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 8, FALSE), "DOCENTE / FACILITADOR"))`
  - `Nombre Centro Costo` (Col G): `=IF(BN=" me","",IFERROR(VLOOKUP(BN, PLANTA_BASE!A:I, 9, FALSE), "GENERAL"))`
...
| :--- | :--- | :--- |
| **Cambiar la estructura visual del HTML o paneles** | `Scripts/procesar_bienvenida.py` | Editar la plantilla `html_content` en Python y ejecutar `ACTUALIZAR_TABLERO.bat`. |
